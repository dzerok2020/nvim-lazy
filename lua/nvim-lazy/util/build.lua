...
function M.keymaps()
  ...
  group = "General"
  dofile(root .. "/lua/nvim-lazy/config/keymaps.lua")
  group = "LSP"
  local lsp = dofile(root .. "/lua/nvim-lazy/plugins/lsp/keymaps.lua")
  ...
  local core = require("lazy.core.plugin").Spec.new({ import = "nvim-lazy.plugins" })
...
```]]):format(
        name,
        Util.read_file(vim.fn.fnamemodify(root .. "/../nvim-lazy-starter/lua/config/" .. name .. ".lua", ":p"))
      ),
      "",
      "</TabItem>",
      ([[<TabItem value="defaults" label="Default %s">]]):format(title),
      "",
      ([[```lua title="nvim-lazy.config.%s"
%s
```]]):format(name, Util.read_file(vim.fn.fnamemodify(root .. "/lua/nvim-lazy/config/" .. name .. ".lua", ":p"))),
      "",
      "</TabItem>",
      "</Tabs>",
      "",
...
function M.update2()
  ...
  Util.walk(root .. "/lua/nvim-lazy/plugins/extras", function(path, name, type)
...
function M.plugins(path)
  local test = root .. "/lua/nvim-lazy/plugins/" .. path
